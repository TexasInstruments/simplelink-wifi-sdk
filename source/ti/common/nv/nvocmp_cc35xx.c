/******************************************************************************
 ...
 @file  nvocmp_cc35xx.c

 @brief NV driver for CC35XX devices - Off-Chip Multi-Page Flash Memory
...
#define NVOCMP_AES_BLOCK_SIZE 16
...
// CC35XX devices flash page size is (1 << 12) or 0x1000
...
#if defined(DeviceFamily_CC23X0R5) || defined(DeviceFamily_CC23X0R53) || defined(DeviceFamily_CC23X0R2) || defined(DeviceFamily_CC23X0R22) || (DeviceFamily_PARENT == DeviceFamily_PARENT_CC27XX) || defined(DeviceFamily_CC35XX)
...
/* CC35XX device does not use NVS APIs. CC35XX device uses the flash APIs
 * directly via the XMEMWFF3 APIs. Mapping these device specific flash APIs
 * below for using them with NVOCMP.
 */
#define NVS_Handle      XMEM_Handle
#define NVS_Attrs       XMEM_Params
#define NVS_init        XMEMWFF3_init
#define NVS_open        XMEMWFF3_open
#define NVS_getAttrs    XMEMWFF3_getAttrs
#define NVS_read        XMEMWFF3_read
#define NVS_write       XMEMWFF3_write
#define NVS_erase       XMEMWFF3_erase
...
static size_t   XMEMWFF3_sectorSize;
...
static uint8_t NVOCMP_initNvApi(void *param)
{
    ...
    if(NVOCMP_failF == NVINTF_NOTREADY)
    {
        ...
        /* Configure XMEM parameters for NVS */
        NVOCMP_nvsAttrs.regionBase      = nvocmp_physical_slot_address;
        NVOCMP_nvsAttrs.regionStartAddr = nvocmp_logical_slot_address;
        NVOCMP_nvsAttrs.regionSize      = nvocmp_region_size;
        NVOCMP_nvsAttrs.deviceNum       = XMEMWFF3_NVS_DEVICE;

        NVOCMP_nvsHandle = (XMEM_Handle)NVS_open(&NVOCMP_nvsAttrs);

        // Get NV hardware attributes
        NVS_getAttrs(NVOCMP_nvsHandle, &NVOCMP_hwAttrs);
        ...
        NVOCMP_nvHandle.nvSize = NVOCMP_nvsAttrs.regionSize/XMEMWFF3_sectorSize;
        ...
        if (FLASH_PAGE_SIZE != XMEMWFF3_sectorSize ||
...
{
#ifndef NV_LINUX
    NVS_read(NVOCMP_nvsHandle, NVOCMP_FLASHOFFSET(pg, off), (uint8_t *)pBuf, len, XMEMWFF3_NVS_FLAGS);
#else
