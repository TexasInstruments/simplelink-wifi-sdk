...
// TI - cc33xx compilation
// moved from wpa_supplicant_driver_init
int interface_count;

extern void gas_query_init_mem();
extern void wpa_debug_init();


// TI - cc33xx compilation - the below must be activated in order to initialized
// supplicant global variables which were originaly initialized in declaration.
void wpa_mem_init()
{
	interface_count = 0;
...
#ifdef CONFIG_TESTING_OPTIONS
	wpa_msg_ctrl(wpa_s, MSG_INFO, "EAPOL-RX " MACSTR " %d",
...
{
        // TI - cc33xx compilation - moved the static to global
...
static void radio_work_free(struct wpa_radio_work *work)
{
	if (work->wpa_s->scan_work == work) {
		...
		wpa_dbg(work->wpa_s, MSG_INFO, "Freeing radio work '%s'@%p (started=%d) that is marked as scan_work",
			work->type, work, (uint32)work->started);
				     ...
				     const struct wpa_interface *iface)
{
	...
	if (iface->confname) {
        ...
        wpa_s->confanother = os_strdup(iface->confanother);
		if (wpa_s->confanother &&
...
// TI - cc33xx compilation - remove static so this function can be called outside
...
void wpas_periodic(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "wpas periodic"); //temp
// TI - cc33xx compilation - don't enable periodic timer, cc33xx cme will use non wake up timer to call the periodic
// cleanup, in order to not to wake up from sleep
#if 1
	wpa_dbg(wpa_s, MSG_DEBUG, "wpas periodic timeout. set new timer, %d seconds",
