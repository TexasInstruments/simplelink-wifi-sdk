...
#define NR_IE_MIN_LEN (ETH_ALEN + 4 + 1 + 1 + 1)
	...
	while (len >= 2 + NR_IE_MIN_LEN) {
		...
		if (2U + nr_len > len) {
			wpa_dbg(wpa_s, MSG_DEBUG,
				"CTRL: Invalid Neighbor Report element: id=%u len=%d nr_len=%u",
				data[0], len, nr_len);
			goto out;
		}
		pos += 2;
				       ...
				       struct type10_params *param)
{
	...
	if (filter_len != (size_t) (end - pos) / 2) {
		wpa_printf(MSG_ERROR,
			   "Filter mask length mismatch expected=%d received=%d",
...
//TI -Compilation - smart config: provisioning solution: added out:
out:
	if (reply_len < 0) {
