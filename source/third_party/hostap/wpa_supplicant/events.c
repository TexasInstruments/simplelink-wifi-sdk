					 ...
					 struct wpa_bss *bss, int debug_print)
{
	...
	while ((ssid->proto & (WPA_PROTO_RSN | WPA_PROTO_OSEN)) && rsn_ie) {
		...
		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			if (debug_print)
		    // TI - cc33xx compilation - improve print
//				wpa_dbg(wpa_s, MSG_DEBUG,
//					"   skip RSN IE - key mgmt mismatch");
		    	wpa_dbg(wpa_s, MSG_DEBUG, "   skip RSN IE - key mgmt mismatch; ie %d, ssid %d",ie.key_mgmt, ssid->key_mgmt);
			break;
		}
...
{
//TI cc33xx compilation - allow scan with no enabled networks when 'select network' is initiated by cc33xx
    //if (!wpa_supplicant_enabled_networks(wpa_s)) {
	if (!wpa_s->cc33xx_initiated_select_network &&
			   ...
			   struct wpa_ssid *ssid)
{
	...
	// TI - cc33xx compilation - adjust print to gtrace
//	wpa_msg(wpa_s, MSG_DEBUG,
...
//		ssid, wpa_s->current_ssid);
	{
		char selected_bssid[20];
		char bssid[20];
		char pending[20];

		sprintf(selected_bssid, MACSTR, MAC2STR(selected->bssid));
		sprintf(bssid, MACSTR, MAC2STR(wpa_s->bssid));
		sprintf(pending, MACSTR, MAC2STR(wpa_s->pending_bssid));
		wpa_msg(wpa_s, MSG_DEBUG, "Considering connect request: reassociate: %d  selected: %s bssid %s pending %s wpa_state %s",
									wpa_s->reassociate, selected_bssid, bssid, pending, wpa_supplicant_state_txt(wpa_s->wpa_state));
	}
...
//TODO: Set Last scan timeout back to 5 sec
        if (os_reltime_expired(&now, &wpa_s->last_scan, 60)) { //TODO: temp
					      ...
					      union wpa_event_data *data)
{
	...
	if (wpa_s->wpa_proto & (WPA_PROTO_RSN | WPA_PROTO_OSEN)) {
		...
		// TI  compilation: RSN override support from upstream - start
	    bss_rsn = wpa_bss_get_rsne(wpa_s, bss, ssid, false); // TI cc33xx compilation - MLO not supported
				       ...
				       union wpa_event_data *data)
{
	...
	// CC33xx compilation - replace wpa_msg with wpa_printf so it can be seen in log file
//	wpa_msg(wpa_s, MSG_INFO, "Associated with " MACSTR, MAC2STR(bssid));
	wpa_printf(MSG_DEBUG, "Associated with " MACSTR, MAC2STR(bssid));
	if (wpa_s->current_ssid) {
						 ...
						 int locally_generated)
{
	...
	//
	//CC33XX code (Oct 26 2016)
	//remove this bug fix as it creates another state transition
	//drom disconnect to AUTH and back to disconnected
	//this is problematic from CME perspective, thinkgs he is already disconnected
	//while supplicant issues another AUTH
	//eliminate this bug fix
				    ...
				    union wpa_event_data *data)
{
    ...
    /*
     *  TI -Compilation - smart config: provisioning solution
     */
static struct wpa_ssid *
