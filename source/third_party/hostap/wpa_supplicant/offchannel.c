...
//			   iface->ifname, wpa_s->ifname);
		wpa_printf(MSG_DEBUG, "P2P: Use group interface %s instead of interface %s for Action TX",
			   iface->ifname, wpa_s->ifname);
		return iface;
	}
...
//			   iface->assoc_freq);
		wpa_printf(MSG_DEBUG, "Off-channel: Pending Action frame TX waiting for another freq=%u (off_channel_freq=%u assoc_freq=%u)",
			   wpa_s->pending_action_freq,
			   wpa_s->off_channel_freq,
			   iface->assoc_freq);
...
//			   (unsigned long long) len);
		wpa_printf(MSG_DEBUG, "Off-channel: Failed to allocate Action frame TX buffer (len=%u)", len);
		return -1;
	}
