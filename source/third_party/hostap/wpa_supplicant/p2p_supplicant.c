				      ...
				      struct wpa_scan_results *scan_res)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (bss->beacon_ie_len > 0 &&
		    ...
		    //TI - cc33xx compilation - adjust log
...
//TI - cc33xx compilation - gtrace can't handle this
//	wpa_printf(MSG_DEBUG,
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	// Originally supplicant can hold a list of 100 devices.
	// This number is reduced due to memory limitations
	// ------------------------------------------------------------------------
	p2p.max_peers = 15;
