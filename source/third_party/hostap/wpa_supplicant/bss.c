...
//			reason);
	{
		...
		sprintf(str, MACSTR, MAC2STR(bss->bssid));
		pStr = wpa_ssid_txt((const u8 *)bss->ssid, bss->ssid_len);
        // TI - cc33xx compilation - fixed reason to be printed with %s (was %d)
        wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Remove id %d due to %s", bss->id, reason);
	}
				    ...
				    struct os_reltime *fetch_time)
{
	...
	if (!is_zero_ether_addr(bss->hessid))
		...
		extra[0] = '\0';
// TI - cc33xx compilation - adjust print to gtrace
	wpa_dbg(wpa_s, MSG_DEBUG, "BSS: Add new id %u BSSID " MACSTR " SSID '%s' num bss %d",
			bss->id, MAC2STR(bss->bssid), wpa_ssid_txt(ssid, ssid_len), wpa_s->num_bss);
...
// TI - cc33xx compilation - adjust print to gtrace
			wpa_dbg(wpa_s, MSG_DEBUG,
					"BSS: Ignore driver BSS " "table entry that is %u.%06u seconds older " "than our scan trigger",
			...
			int new_scan)
{
	...
	wpa_printf(MSG_DEBUG, "BSS: last_scan_res_used=%d/%d",
