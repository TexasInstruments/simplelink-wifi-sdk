...
extern void* eapol_sm_get_priv_method(struct eapol_sm *sm);
extern struct wps_data* eap_wsc_get_wps_data(void *priv);

int wpas_wps_eapol_cb(struct wpa_supplicant *wpa_s)
{
    /***********************************************/
    // TI - cc33xx compilation - added to free the data here so GLX retention in WPS connection setup can be used
    struct eapol_sm *eapol = wpa_s->eapol;
    void *priv = eapol_sm_get_priv_method(eapol);
    struct wps_data *wps_d = NULL;

    if(priv){
        wps_d = eap_wsc_get_wps_data(priv);
    }
    /***********************************************/
	...
	if (!wpa_s->wps_success &&
        ...
        // TI - cc33xx compilation - free the data here so GLX retention in WPS connection setup can be used
		if (wps_d){
	        wps_free_temp_data(wps_d);
		}
				   ...
				   const struct wps_credential *cred)
{
	...
	if ((wpa_s->conf->wps_cred_processing == 1 ||
		...
		// TI - cc33xx compilation - notification will be sent after the internal processing of the credentials by the supplicant, at the
		//                           end of the function.
	...
	// TI - cc33xx compilation
	// Credentials are passed here, after the credentials were checked and parsed by the supplicant in the above code, so the driver
	// can use directly the parsed information from wpa_s->current_ssid (like ciphers).
	if (wpa_s->conf->wps_cred_processing == 2)
	{
	    wpas_notify_wps_credential(wpa_s, cred);
	}
...
//TI cc33xx compilation - remove registrar for code reduction
