...
/*
 * cc33xx changes: Activate driver_cc33 API when WLAN state is changed.
 *                 Notify WPS events.
 *
 */
...
// TI - cc33xx compilation - notifications APIs
extern void ti_driver_state_changed(struct wpa_supplicant *wpa_s,
                                      enum wpa_states new_state,
                                      enum wpa_states old_state);
extern void cc33xx_driver_wps_success(struct wpa_supplicant *wpa_s);
extern void cc33xx_driver_wps_fail(struct wpa_supplicant *wpa_s);
extern void cc33xx_driver_wps_cleared(struct wpa_supplicant *wpa_s);
extern void cc33xx_driver_wps_credential(struct wpa_supplicant *wpa_s, const struct wps_credential *cred);
extern void ti_driver_p2p_dev_found(struct wpa_supplicant *wpa_s,const uint8_t *peer_addr);
extern void ti_driver_p2p_find_stopped(struct wpa_supplicant *wpa_s);
extern void ti_driver_go_neg_completed(struct wpa_supplicant *wpa_s,
                                       struct p2p_go_neg_results *res);
extern void ti_driver_p2p_group_formation_failure(struct wpa_supplicant *wpa_s);
extern void ti_driver_p2p_group_started(struct wpa_supplicant *wpa_s, struct wpa_ssid *ssid, uint8_t client);
extern void ti_driver_p2p_group_removed(struct wpa_supplicant *wpa_s);
			       ...
			       enum wpa_states old_state)
{
	...
	// TI -  compilation - notify driver in case of connection state is changed
	ti_driver_state_changed(wpa_s, new_state, old_state);
}
				...
				const struct wps_credential *cred)
{
    ...
    // TI - cc33xx compilation
	cc33xx_driver_wps_credential(wpa_s, cred);
...
void wpas_notify_wps_event_success(struct wpa_supplicant *wpa_s)
{
    ...
    // TI - cc33xx compilation
    cc33xx_driver_wps_success(wpa_s);
...
// TI - cc33xx compilation
// WPS process was cleared (cancelled or timed-out)
void wpas_notify_wps_cleared(struct wpa_supplicant *wpa_s)
{
#ifdef CONFIG_WPS
    cc33xx_driver_wps_cleared(wpa_s);
...
void wpas_notify_p2p_find_stopped(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "notify_p2p_find_stopped");
	ti_driver_p2p_find_stopped(wpa_s);
}
				  ...
				  const u8 *dev_addr, int new_device)
{
    ...
    wpa_printf(MSG_DEBUG, "notify_p2p_device_found: dev " MACSTR ,
		                MAC2STR(dev_addr));
    ti_driver_p2p_dev_found(wpa_s, dev_addr);
}
				   ...
				   const char *role)
{
	...
	wpa_printf(MSG_DEBUG, "notify: p2p_group_removed! Role = %s", role);
	wpas_dbus_unregister_p2p_group(wpa_s, ssid);
	ti_driver_p2p_group_removed(wpa_s);
}
				      ...
				      struct p2p_go_neg_results *res)
{
	...
	wpa_printf(MSG_DEBUG, "notify_p2p_go_neg_completed: status=%d", res->status);
	ti_driver_go_neg_completed(wpa_s, res);
}
					 ...
					 unsigned int generated_pin)
{
	wpas_dbus_signal_p2p_provision_discovery(wpa_s, dev_addr, request,
						 ...
						 generated_pin);
    wpa_printf(MSG_DEBUG, "notify: p2p_provision_discovery: request=%d status=%d config_methods=0x%x generated_pin=%d", 
                            request, status, config_methods, generated_pin);
}
				   ...
				   int client, const u8 *ip)
{
    ...
    wpa_printf(MSG_DEBUG, "notify: p2p_group_started: client=%d ssid=%s, bssid=0x%x", 
			client, ssid->ssid, ssid->bssid[0]);
    ti_driver_p2p_group_started(wpa_s, ssid, client);
}
					     ...
					     const char *reason)
{
    ...
    wpa_printf(MSG_DEBUG, "notify: p2p_group_formation_failure: %s ", reason);
    ti_driver_p2p_group_formation_failure(wpa_s);
}
				...
				struct wps_event_fail *fail)
{
    ...
    wpa_printf(MSG_DEBUG, "notify: p2p_wps_failed");
}
