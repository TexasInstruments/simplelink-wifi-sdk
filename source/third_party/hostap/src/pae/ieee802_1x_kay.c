...
ieee802_1x_mka_dump_basic_body(struct ieee802_1x_mka_basic_body *body)
{
	...
	wpa_printf(MSG_DEBUG, "\tParameter set body length: %d", body_len);
				 ...
				 size_t msg_len)
{
	...
	if (body_len < sizeof(struct ieee802_1x_mka_basic_body) - MKA_HDR_LEN) {
		wpa_printf(MSG_DEBUG, "KaY: Too small body length %d",
			   body_len);
		return NULL;
	}
	ckn_len = body_len -
			     ...
			     const u8 *mka_msg, size_t msg_len)
{
	...
	for (pos = mka_msg, left_len = msg_len;
		...
		if ((body_len % 16) != 0) {
			wpa_printf(MSG_ERROR,
				   "KaY: MKA Peer Packet Body Length (%d bytes) should be a multiple of 16 octets",
				   body_len);
			continue;
		}
	...
	const u8 *mka_msg, size_t msg_len)
{
	...
	if (body_len < 28) {
		wpa_printf(MSG_ERROR,
			   "KaY: MKA Use CAK Packet Body Length (%d bytes) should be 28 or more octets",
			   body_len);
		return -1;
	}
	...
	const u8 *mka_msg, size_t msg_len)
{
	...
	if (body_len < 5) {
		wpa_printf(MSG_ERROR,
			   "KaY: MKA Use KMD Packet Body Length (%d bytes) should be 5 or more octets",
			   body_len);
		return -1;
	}
					       ...
					       const u8 *buf, size_t len)
{
	...
	if (ckn_len < 1 || ckn_len > MAX_CKN_LEN) {
		wpa_printf(MSG_WARNING,
			   "KaY: Received EAPOL-MKA CKN Length (%d bytes) is out of range (<= %u bytes)",
			   ckn_len, MAX_CKN_LEN);
		return -1;
	}
