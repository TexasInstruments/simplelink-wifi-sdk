				   ...
				   enum key_flag key_flag)
{
	...
	return wpa_auth->cb->set_key(wpa_auth->cb_ctx, vlan_id, alg, addr, idx,
				     key, key_len, key_flag);//hostapd_wpa_auth_set_key
}
		 ...
		 u8 *data, size_t data_len)
{
	...
	wpa_printf(MSG_DEBUG, "WPA: Received EAPOL-Key from " MACSTR
		   " key_info=0x%x type=%u mic_len=%d key_data_length=%u",
	...
	if (key_data_length > data_len - sizeof(*hdr) - keyhdrlen) {
		//wpa_printf(MSG_INFO,
		//	   "WPA: Invalid EAPOL-Key frame - key_data overflow (%d > %d)",
		//	   key_data_length,
		      ...
		      int keyidx, int encr, int force_version)
{
	...
	wpa_printf(MSG_DEBUG,
		   "WPA: Send EAPOL(version=%d secure=%d mic=%d ack=%d install=%d pairwise=%d kde_len=%d keyidx=%d encr=%d)",
		   version,
	...
	if (kde && !encr) {
		...
		if (version == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
		    ...
		    version == WPA_KEY_INFO_TYPE_AES_128_CMAC) {
			wpa_printf(MSG_DEBUG,
				   "WPA: Encrypt Key Data using AES-WRAP (KEK length %d)",
				   sm->PTK.kek_len);
...
SM_STATE(WPA_PTK, INITPMK)
{
	...
	if (sm->pmksa) {
		...
		if (wpa_key_mgmt_sha384(sm->wpa_key_mgmt))
			...
			pmk_len = PMK_LEN;
		wpa_printf(MSG_DEBUG,
			   "WPA: PMK from EAPOL state machine (MSK len=%d PMK len=%u)",
			   len, pmk_len);
		if (len < pmk_len) {
			wpa_printf(MSG_DEBUG,
				   "WPA: MSK not long enough (%d) to create PMK (%u)",
				   len, pmk_len);
			sm->Disconnect = true;
			return;
		}
				    ...
				    const u8 *fils_session, struct wpabuf *hlp)
{
	...
	*pos++ = WLAN_EID_EXT_FILS_SESSION; /* Element ID Extension */
	...
	wpa_printf(MSG_DEBUG, "%s: plain buf_len: %d", __func__,
		   wpabuf_len(plain));
	wpabuf_clear_free(plain);
	sm->fils_completed = 1;
	return pos;
