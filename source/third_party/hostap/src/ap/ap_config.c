...
//ti compilation - removed (not compiling with netinet libraries
...
//TI - cc33xx compilation - AP role - use glx
//TODO: Is this correct in case of mesh and P2P? need to re-check
...
#define ecw2cw(ecw) ((1 << (ecw)) - 1)
    ...
    // TODO - Gil - is there another way to config this?
    ////TI - cc33xx compilation - changed txop limit to 94
	const struct hostapd_tx_queue_params txq_vi =
		{ 1, (ecw2cw(aCWmin) + 1) / 2 - 1, ecw2cw(aCWmin), 94};
	////TI - cc33xx compilation - changed txop limit to 47
	const struct hostapd_tx_queue_params txq_vo =
		{ 1, (ecw2cw(aCWmin) + 1) / 4 - 1,
		  (ecw2cw(aCWmin) + 1) / 2 - 1, 47};
...
#undef ecw2cw
	...
	//TI- cc33xx compilation : this is being overide by our parametres in l2cfg
	conf->wmm_ac_params[0] = ac_be;
	conf->wmm_ac_params[1] = ac_bk;
	conf->wmm_ac_params[2] = ac_vi;
	conf->wmm_ac_params[3] = ac_vo;

    //TI- cc33xx compilation - this parameters are to set the txop for our AP tx queues
	...
	// TI - cc33xx compilation
        // TODO - config via conf mechanism?
	conf->ap_table_max_size = 10; //255;
...
#define TI_80211D_SUPPORT
#ifdef TI_80211D_SUPPORT
	uint8_t localCountryDomainStr[3]= {0} ;
	extern int32_t regulatoryDomain_getCountryString(uint8_t *pCountryStr);

	regulatoryDomain_getCountryString(localCountryDomainStr);

	conf->country[0]= localCountryDomainStr[0];
	conf->country[1]= localCountryDomainStr[1];
	conf->country[2]= localCountryDomainStr[2];
	conf->ieee80211d = 1;
#endif // TI_80211D_SUPPORT
			   ...
			   const u8 *prev_psk, int *vlan_id)
{
	...
	if (p2p_dev_addr && !is_zero_ether_addr(p2p_dev_addr)) {
	    // TI - cc33xx compilation - Gtrace does not recognize %p (it is assumed as string and in case it is NULL it crashes on the strlen)
	    // cast it to uint32.
...
#endif /* CONFIG_WEP */
	//TI -Compilation - based on MCP version 8.0.0.48
	if (full_config && conf->ieee80211n &&
		conf->channel == 14){
		bss->disable_11n = 1;
		wpa_printf(MSG_ERROR, "HT (IEEE 802.11n) with channel 14 is not "
				"allowed, disabling HT capabilities");
	}
