				     ...
				     struct wpabuf *wd)
{
	...
	if (buf_len < 6) {
		wpa_printf(MSG_DEBUG, "PASN: SAE buffer too short. len=%d",
			   buf_len);
		return -1;
	}
			       ...
			       struct wpabuf *wd)
{
	...
	if (buf_len < 6) {
		wpa_printf(MSG_DEBUG, "PASN: FILS: Buffer too short. len=%d",
			   buf_len);
		return -1;
	}
			       ...
			       const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	if (hapd->conf->force_kdk_derivation ||
		...
		sta->pasn->kdk_len = 0;
	wpa_printf(MSG_DEBUG, "PASN: kdk_len=%d", sta->pasn->kdk_len);
			      ...
			      struct sta_info *sta, int reassoc)
{
	...
	wpa_printf(MSG_DEBUG, "Add associated STA " MACSTR
		   ...
		   MAC2STR(sta->addr), (uint32)sta->added_unassoc, sta->auth_alg,
		   (uint32)sta->ft_over_ds, reassoc,
