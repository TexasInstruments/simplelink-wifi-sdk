...
//for compilers that don't have errno.h we use user_errno.h
#if __has_include("errno.h") && __has_include(<errno.h>)
# include <errno.h>
#endif
...
//========================TEMP COPY CMP ZERE MAC ADDR =====================//
static void inline IRQ_UtilCopyMacAddress  ( const uint8 source_address[6] ,
                                             uint8 dest_address  [6] )
{
    os_memcpy(dest_address, source_address, 6);
}

static int inline  IRQ_UtilCompareMacAddress  ( const uint8 address1[6] ,
                                                const uint8 address2[6] )
{
    return (os_memcmp(address1, address2, 6) == 0);

}

static void inline IRQ_UtilZeroMacAddress(uint8 address[6])
{
    os_memset(address, 0, 6);
}

static int inline IRQ_UtilIsZeroMacAddress(const uint8 address1[6])
{
	const uint8 address2[6] = {0};
    return IRQ_UtilCompareMacAddress(address1, address2);
}
