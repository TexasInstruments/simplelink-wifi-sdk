...
int x509_parse_time(const u8 *buf, size_t len, u8 asn1_tag, os_time_t *val)
{
    ...
    // need to implement ?? if (os_mktime(year, month, day, hour, min, sec, val) < 0)  - from original code
#if 0
        // TI cc33xx compilation
	/* time is initialized so that os_mktime can be used */
