...
int crypto_bignum_rand(struct crypto_bignum *r, const struct crypto_bignum *m)
{
    ...
    prime_bits = mp_count_bits((mp_int *)m);
    prime_len = ((mp_count_bits((mp_int *)m) + 7) / 8);
    ...
    if(wc_RNG_GenerateBlock(&rng, tmp, prime_len) < 0)
    {
        return -1;
    }
    ...
    mp_read_unsigned_bin((mp_int *)r, tmp,prime_len);
