...
// TI - cc33xx compilation - added  get function for eap_wsc_data -> wps member
struct wps_data* eap_wsc_get_wps_data(void *priv){
				       ...
				       const struct wpabuf *reqData)
{
	...
	case WPS_DONE:
		...
		eap_wsc_state(data, FAIL);
		// TI - cc33xx compilation - free the data here so GLX retention in WPS connection setup can be used (this state seems for registrar)
		wps_free_temp_data(data->wps);
		break;
	case WPS_CONTINUE:
		eap_wsc_state(data, MESG);
		break;
...
send_msg:
	...
	if (data->state == FAIL && ret->methodState == METHOD_DONE) {
        ...
        // TI - cc33xx compilation - wps completed, free the data here so GLX retention in WPS connection setup can be used
        wps_free_temp_data(data->wps);
	}
	return r;
}
