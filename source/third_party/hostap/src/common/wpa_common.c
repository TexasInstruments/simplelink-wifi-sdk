...
#define MAX_Z_LEN 66 /* with NIST P-521 */
	...
	if (kdk_len > WPA_KDK_MAX_LEN) {
		wpa_printf(MSG_ERROR,
			   "WPA: KDK len=%d exceeds max supported len",
			   kdk_len);
		return -1;
	}
		    ...
		    u8 *fils_ft, size_t *fils_ft_len, size_t kdk_len)
{
	...
	if (kdk_len) {
		if (kdk_len > WPA_KDK_MAX_LEN) {
			wpa_printf(MSG_ERROR, "FILS: KDK len=%d too big",
				   kdk_len);
			goto err;
		}
			 ...
			 struct wpa_ie_data *data)
{
	...
	if (rsn_ie_len >= 6 && rsn_ie[1] >= 4 &&
	    ...
	    // TI - cc33xx compilation - improve log
//		wpa_printf(MSG_DEBUG, "%s: malformed ie or unknown version",
//			   __func__);
	    wpa_printf(MSG_DEBUG, "RSN IE error: hdr->elem_id %d hdr->len %d rsn_ie_len %d version %d",
	            hdr->elem_id, hdr->len, rsn_ie_len, WPA_GET_LE16(hdr->version));
			return -2;
		}
		      ...
		      size_t kdk_len)
{
	...
	if (kdk_len > WPA_KDK_MAX_LEN) {
		wpa_printf(MSG_ERROR,
			   "FT: KDK len=%d exceeds max supported len",
			   kdk_len);
		return -1;
	}
