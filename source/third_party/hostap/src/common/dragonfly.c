...
dragonfly_get_rand_1_to_p_1(const struct crypto_bignum *primeBigNum,size_t prime_len, size_t prime_bits, int *r_odd)
{
    ...
    uint8_t prime[66];
    if(crypto_bignum_to_bin(primeBigNum, prime,sizeof(prime), prime_len) < 0)
    {
        return NULL;
    }
    ...
    for (;;) {
        ...
        /* call "random_get_bytes" from some unclear reason change the ROM code
         *  (change on cinit table the order of 3 globals) so this function is called by address  */
        if (os_get_random(tmp, prime_len) < 0)
            break;
        if (prime_bits % 8)
					 ...
					 const struct crypto_bignum *val)
{
	...
	bits = crypto_ec_prime_len_bits(ec);
	 ...
	 * num = (v * r * r) modulo p
	 */
	r = dragonfly_get_rand_1_to_p_1(prime, prime_len, bits, &r_odd);
	if (!r)
		return -1;
