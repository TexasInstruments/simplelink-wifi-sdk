...
static void eapol_port_timers_tick(void *eloop_ctx, void *timeout_ctx)
{
	...
	wpa_printf(MSG_DEBUG, "eapol timeout !!!!");
		      ...
		      size_t len)
{
	...
	case IEEE802_1X_TYPE_EAP_PACKET:
		...
		wpabuf_free(sm->eapReqData);
		//TODO: Move to glx! Check when is this being used.
		sm->eapReqData = wpabuf_alloc_copy(hdr + 1, plen); //TODO: Supplicant free this only when getting new eap packet or disconnect
		                                                   //this prevents us to put it in glx - check if we can free this when we finish
		                                                   //processing it, and move it to glx.
		if (sm->eapReqData) {
...
// TI - cc33xx compilation - added  get function for wpas_wps_eapol_cb()
void * eapol_sm_get_priv_method(struct eapol_sm *sm){
    void * ret = NULL;
    if (sm){
        // need to implement
#if 0
        ret = sm->eap->eap_method_priv;
#endif
    }
    return ret;
}
